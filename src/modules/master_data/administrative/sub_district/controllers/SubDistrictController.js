...
const DistrictModel = require("../../district/models/DistrictModel");
...
exports.CreateSubDistrict = async (req, res) => {
    ...
    try {
        let ID = req.body.district_code;
        const checkId = await checkCodeIsExist(req, res, DistrictModel, ID);
        if (checkId == null) {
            throw new Error('Data District Code not found in database');
        }
        const data = await CreateService(req, res, SubDistrictModel);
...
exports.GetAllSubDistrict = async (req, res) => {
    ...
    try {
        ...
        let results = await GetAllService(req, res, SubDistrictModel, Projection)
        const response = {
            data: results[0].data,
...
exports.UpdateSubDistrict = async (req, res) => {
    ...
    try {
        ...
        let districtCode = req.body.district_code;
        const checkDistrict = await checkCodeIsExist(req, res, DistrictModel, districtCode);
        ...
        await UpdateService(req, res, SubDistrictModel, PostBody)
        ...
        const Projection = [
            {
                $facet: {
                    data: [
                        {
                            $match: { district_code: req.query.id }
                        },
                        {
                            $project: {
                                name: 1,
